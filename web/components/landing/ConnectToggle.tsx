...
/**
 * A tactile, hardware-style toggle button — a "connect" switch with a glowing
 * LED, inset-embossed Bluetooth glyph, and ambient shine. Purely presentational:
 * it toggles its own checked state and lights up, adding a living, pressable
 * element to the hero.
 *
 * All visual styling lives in `globals.css` under the `.connect-toggle` scope.
 */
export function ConnectToggle({ className }: { className?: string }) {
  return (
    <label htmlFor="connect-toggle" className={cn("wrap", className)}>
      <input id="connect-toggle" aria-label="Connect" type="checkbox" />
