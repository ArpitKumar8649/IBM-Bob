...
export const PricingCard = ({
  ...
  const cardClasses = `
    ...
    ${isPopular ? "scale-105 relative ring-2 ring-rose-400/20 dark:from-white/20 dark:to-white/10 dark:border-rose-400/30 shadow-2xl" : ""}
  `;
  const buttonClasses = `
  ...
  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-rose-500 text-white">
          Most Popular
        </div>
      )}
      <div className="mb-3">
      ...
      <ul className="flex flex-col gap-2 text-[14px] text-foreground/90 mb-6 font-sans">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center gap-2">
            <CheckIcon className="text-rose-400 w-4 h-4" /> {feature}
          </li>
        ))}
      </ul>
    ...
    <div className="bg-background text-foreground min-h-screen w-full overflow-x-hidden">
      ...
      <main className="relative w-full min-h-screen flex flex-col items-center justify-center px-4 py-8">
        <div className="w-full max-w-5xl mx-auto text-center mb-14">
          <h1 className="text-[48px] md:text-[64px] font-extralight leading-tight tracking-[-0.03em] bg-clip-text text-transparent bg-gradient-to-r from-slate-900 via-rose-500 to-rose-600 dark:from-white dark:via-rose-300 dark:to-rose-400 font-display">
            {title}
          </h1>
